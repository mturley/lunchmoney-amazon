...
for (const uncategorizedLunchMoneyAmazonTransaction of lunchMoneyAmazonTransactions) {
  ...
  if (orderIsGift(matchingAmazonTransaction)) {
    ...
    const matchingKey = Object.keys(categoryRules).find(
      (key) => matchingAmazonTransaction.categories?.startsWith(key)
    );
  ...
  const newNote = `${matchingAmazonTransaction.items} - #${
    matchingAmazonTransaction.orderid
  } ${uncategorizedLunchMoneyAmazonTransaction.notes || ""}`.trim();
